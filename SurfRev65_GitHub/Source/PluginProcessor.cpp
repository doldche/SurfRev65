...
juce::AudioProcessorValueTreeState::ParameterLayout SurfRev65Processor::createLayout()
{
    ...
    p.push_back (std::make_unique<juce::AudioParameterFloat>
        ("reverb","Reverb", juce::NormalisableRange<float>(0.f,10.f,0.01f), 5.f));
    p.push_back (std::make_unique<juce::AudioParameterFloat>
        ("tone",  "Tone",   juce::NormalisableRange<float>(0.f,10.f,0.01f), 5.f));
    p.push_back (std::make_unique<juce::AudioParameterFloat>
        ("level", "Level",  juce::NormalisableRange<float>(0.f,10.f,0.01f), 7.f));
    p.push_back (std::make_unique<juce::AudioParameterFloat>
        ("gain",  "Input Gain", juce::NormalisableRange<float>(0.5f,20.f,0.1f), 4.f));
    p.push_back (std::make_unique<juce::AudioParameterChoice>
        ("mode",  "Mode",   juce::StringArray{"Spring","Room","Hall"}, 0));
    p.push_back (std::make_unique<juce::AudioParameterBool>
        ("trem",  "Tremolo", false));
    p.push_back (std::make_unique<juce::AudioParameterBool>
        ("boost", "Boost +6dB", false));
    p.push_back (std::make_unique<juce::AudioParameterBool>
        ("locut", "Lo-Cut 180Hz", false));
    p.push_back (std::make_unique<juce::AudioParameterBool>
        ("bypass","Bypass", false));
...
//==============================================================================
// Generacion procedural del impulso de resorte — corazon del algoritmo
//==============================================================================
void SurfRev65Processor::rebuildIR (Mode m, double sr)
{
    ...
    const Cfg cfgs[3] = {
        // Spring — metalico, larga cola, resonancias caracteristicas
        { 3.8f, 2.5f, {{750,0.10f,5},{1100,0.07f,4},{380,0.05f,3},
                        {550,0.04f,3.5f},{920,0.03f,4.5f}} },
        // Room — corta, natural
        { 1.5f, 4.5f, {{400,0.06f,6},{900,0.04f,5},{250,0.03f,7}} },
        // Hall — larga, difusa
        { 5.5f, 1.2f, {{300,0.08f,3},{600,0.05f,2.5f},
                        {1200,0.04f,4},{180,0.06f,2}} }
    };
    ...
    for (int ch = 0; ch < 2; ++ch)
    {
        ...
        for (int i = 0; i < len; ++i)
        {
            ...
            for (auto& r : c.res)
                s += std::sin (juce::MathConstants<float>::twoPi * r.freq * (t + chOff))
                     * r.amp * std::exp (-t * r.decay);
...
//==============================================================================
void SurfRev65Processor::processBlock (juce::AudioBuffer<float>& buffer,
                                        juce::MidiBuffer&)
{
    ...
    if (needRebuild)
    {
        rebuildIR (currentMode, currentSR);
        needRebuild = false;
    }
    ...
    // Leer parametros de forma atomica (thread-safe)
    ...
    if (locut)
    {
        ...
        lcL.coefficients = lcc;
        lcR.coefficients = lcc;
        for (int i = 0; i < N; ++i)
        {
    ...
    // Soft clip para evitar distorsion digital dura
    for (int ch = 0; ch < CH; ++ch)
    {
...
void SurfRev65Processor::setCurrentProgram (int idx)
{
    ...
    const Pre pres[4] = {
        {6,5.5f,7,4,   0,false,false,false},  // Surf Clasico
        {4,6,   7.5f,3,1,false,false,true },  // Ballad Room
        {8,4.5f,6.5f,3.5f,2,false,false,true},// Big Hall
        {5,6,   7,4,   0,true, false,false}   // Tremolo Vintage
    };
    auto& p = pres[idx];
    ...
    if (auto* m = apvts.getParameter("mode"))
        m->setValueNotifyingHost (p.mode / 2.f);
...
void SurfRev65Processor::parameterChanged (const juce::String& id, float v)
{
    if (id == "mode")
    {
        currentMode  = static_cast<Mode> (static_cast<int>(v));
        needRebuild  = true;
    }
}
